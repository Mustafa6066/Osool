...
def determine_strategy(
    ...
    # ═════════════════════════════════════════════════════════════════
    # V5: MISSING EGYPTIAN MARKET-SPECIFIC STATE HANDLERS
    # ═════════════════════════════════════════════════════════════════

    elif state == PsychologicalState.INSTALLMENT_ANXIETY:
        strategy = Strategy.FINANCIAL_REASSURANCE
        angle = "payment_comfort"
        talking_points = [
            "فاهم القلق ده — بس خليني أفصلهالك: القسط الشهري أقل من إيجار شقة في نفس المنطقة.",
            "التقسيط على 8 سنين يعني التضخم بيشتغل لصالحك — القسط بيقل قيمته الحقيقية كل سنة.",
            "المقدم 10% بس، والباقي بدون فوايد عند المطور. يعني مفيش بنك في الموضوع.",
            "لو القسط تقل، عندنا وحدات أصغر بنفس الموقع تبدأ من سعر أقل.",
            "خليني أحسبلك: لو دخلك الشهري X، القسط مش هياخد أكتر من 30% منه."
        ]

    elif state == PsychologicalState.DELIVERY_FEAR:
        strategy = Strategy.TRUST_BUILDING
        angle = "delivery_guarantee"
        talking_points = [
            "حقك تقلق — التأخير في التسليم مشكلة حقيقية في السوق المصري.",
            "علشان كده أنا بفلتر المطورين على أساس سجل التسليم. المطور ده سلم 95% في الوقت.",
            "لو عايز أمان أكتر، عندنا وحدات resale جاهزة — تستلم فوراً صفر مخاطر.",
            "أنا مش هرشحلك مطور عنده تاريخ تأخير. ده مبدئي.",
            "تحب أوريك مشاريع المطور ده اللي اتسلمت بالفعل؟ عندي صور وتقييمات."
        ]

    elif state == PsychologicalState.CURRENCY_HEDGER:
        strategy = Strategy.ROI_FOCUSED
        angle = "currency_protection"
        talking_points = [
            "ذكي إنك بتفكر كده. الجنيه خسر أكتر من 50% من قيمته في 3 سنين.",
            "العقار في مصر بيتصرف زي الدولار — بيحافظ على قيمته وبيزيد.",
            "حضرتك بتشتري أصل بالجنيه سعره مرتبط بالدولار (مواد البناء مستوردة).",
            "العائد الحقيقي: 30% نمو سنوي + إيجار 7.5% = أكتر من أي bank CD.",
            "تحب أوريك الـ Inflation Killer Chart؟ بيوضح الفرق بالأرقام."
        ]

    elif state == PsychologicalState.INFLATION_REFUGEE:
        strategy = Strategy.REPLACEMENT_COST_PITCH
        angle = "wealth_shield"
        talking_points = [
            "أنا فاهم — كل يوم الفلوس بتخسر قيمتها. ده الواقع.",
            "العقار هو الطريقة الوحيدة إنك 'تجمد' قيمة فلوسك بل وتزودها.",
            "تكلفة البناء النهاردة 15,000 جنيه/متر — يعني حتى لو السوق بطأ، العقار مش هينزل تحت كده.",
            "الشهادة بتديك 22% فايدة، بس التضخم 13.6% بياكل معظمها. العقار 30% نمو + 7.5% إيجار.",
            "المعادلة بسيطة: كاش = خسارة مستمرة. عقار = protection + نمو."
        ]
