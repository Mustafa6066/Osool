      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        {projects.length > 0 ? (
                  ...
                  return (
                    <>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {p.status && (
                          <span className="rounded-full bg-[var(--color-primary-dark)]/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--color-primary-dark)] dark:text-[var(--color-primary-light)]">
                            {p.status}
                          </span>
                        )}
                      </div>
          ...
          <div className="rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-8 text-center">
            <h2 className="mb-2 text-lg font-semibold"><T k="projPage.heroTitle" /></h2>
            <p className="mx-auto max-w-2xl text-sm text-[var(--color-text-muted)]">
              {loadError
                ? 'Project data is temporarily unavailable. Please refresh in a moment.'
                : 'No projects are available yet. Check back shortly for updated listings.'}
            </p>
          </div>
        )}
      </div>
