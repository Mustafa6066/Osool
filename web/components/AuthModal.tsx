...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
    ...
    const handleWalletSuccess = async () => {
        ...
        try {
            ...
            // If new user logic is needed, we handle it. 
            // For now, assume success closes modal.
            onSuccess();
            onClose();
    ...
    const handleEmailAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (activeTab === "signup") {
                // SIGNUP (Query Params based on current endpoints.py)
                ...
                const res = await fetch(`${API_URL}/api/auth/signup?${params.toString()}`, {
                    method: "POST"
                });
    ...
    const loginUser = async () => {
        ...
        const data = await res.json();
        localStorage.setItem("osool_jwt", data.access_token);
        localStorage.setItem("osool_role", "investor"); // simplified
        onSuccess();
        onClose();
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                    <h2 className="text-xl font-bold text-gray-900">Sign in to Osool</h2>
